 ...
 * 书架配置：数据已拆出到 assets/courses.json（纯 JSON）。
 * 以后加书 / 加章节 / 挂笔记只改那个文件即可，不用再动 JS。
 * 结构字段说明见 blog/README.md「书架与知识结构」。
 * 渲染逻辑继续读全局 COURSES，本文件只负责把它拉进来。
 * ===================================================== */
let COURSES = [];
...
async function initPost() {
  ...
  // 书架配置（用于“返回书架”链接）先拉一次；失败只影响该链接
  try { await getCourses(); } catch (e) { COURSES = []; }
...
if (page === "index") {
  ...
  // 书架配置在 assets/courses.json 里，先拉取再渲染；失败时书架为空并提示
  getCourses().catch(() => { COURSES = []; }).then(() => renderRoute());
} else if (page === "post") {
  initPost();
}
